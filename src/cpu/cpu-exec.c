...
bool is_trigged_by_watchpoints() {
  for (int i = 0; i < NR_WP; i++) {
    WP cur = wp_pool[i];
    if (cur.state == STATE_USED) {
      bool success = false;
      if (expr(cur.what, &success) == 0 && success == true) {
        return true;
      }
    }
  }
...
static void trace_and_difftest(Decode *_this, vaddr_t dnpc) {
  ...
  if (is_trigged_by_watchpoints() == true) {
...
static void statistic() {
  IFNDEF(CONFIG_TARGET_AM, setlocale(LC_NUMERIC, ""));  // TODO
